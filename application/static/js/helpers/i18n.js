...
function (formHelper, wysihtml5) {
	...
	var submitForm = function ($form, action, successCallback, failCallback, dataFormatter) {
		var formatter;
		if (dataFormatter) {
			formatter = function (data) { return dataFormatter(formatI18nData(data)); };
		} else {
			formatter = formatI18nData;
		}
		// submit
		formHelper.submitForm($form, action, successCallback, failCallback, formatter);
	};

	var translate = function (what, langId) {
		if (!langId) {
	...
	var renderLanguageTabs = function ($context, TAB_ID_BASE) {
	...
	return {
		 ...
		 */
		'renderLanguageTabs': renderLanguageTabs
	};
